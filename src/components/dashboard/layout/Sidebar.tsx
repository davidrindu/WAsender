...
const Sidebar = ({
  ...
  const getActiveItem = () => {
    ...
    // Check if current path matches any nav item path
    const matchedNavItem = [...items, ...defaultBottomItems].find(
      (item) =>
        item.path &&
        (currentPath === item.path || currentPath.startsWith(item.path + "/")),
    );

    return matchedNavItem?.label || (currentPath === "/" ? "Home" : "");
  };
        ...
        <div className="space-y-1.5">
          {items.map((item) => (
            <Button
              key={item.label}
              variant={"ghost"}
              className={`w-full justify-start gap-3 h-10 rounded-xl text-sm font-medium ${item.label === currentActiveItem ? "bg-blue-50 text-blue-600 hover:bg-blue-100" : "text-gray-700 hover:bg-gray-100"}`}
              onClick={() => {
            ...
            >
              <span
                className={`${item.label === currentActiveItem ? "text-blue-600" : "text-gray-500"}`}
              >
                {item.icon}
              </span>
              {item.label}
